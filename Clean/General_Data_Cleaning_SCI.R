...
p_load(tidyverse, iotools, data.table, haven, knitr, psych, stats4, stargazer, magrittr, 
       ...
       ggplot2, ggridges, plotly, openxlsx, dplyr, readr, zoo,disprose, stringr,covidcast)
...
# save data frames
saveRDS(df.cumulative.dist.wide, file = "Data/Temp/df_cumulative_distribution_state.rds")
saveRDS(df.1diff.dist.wide, file = "Data/Temp/df.1diff.distribution_state.rds")
...
df.friendvax.dist <- FriendTimesVax(df.friendweights, df_vaxrate)
#df.friendvax.dist <- FriendTimesVax(df.friendweights, df.1diff.dist.wide)
# df.friendvax.vaxrate <- FriendTimesVax(df.friendnum, )

saveRDS(df.friendvax.dist, file = 'Data/Temp/friendvax_state.rds')
...
# save wide table
saveRDS(df.state.dist, file = 'Data/Final Data/wide table_N=92356_v0629.rds')
...
saveRDS(df.state.dist.long, file = 'Data/Final Data/long table_N=92356_v0629.rds')
