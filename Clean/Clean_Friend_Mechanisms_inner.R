...
df.all$inner <- +(df.all$user_id_following %in% inner_id)
df_inner <- df.all[df.all$inner == 1]

df.inner <- subset(df_inner, select = -c(nation, admin1Abb,user_id_following,admin1,admin3))
df.collapse.inner <- df.inner %>% group_by(user_id) %>% summarise_all(sum)
...
df.friendvax.inner <- MultiplyNumVax(df.collapse.cele, df.vaxsupply)
