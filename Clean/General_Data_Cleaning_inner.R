...
inner_id <- df_uservax$user_id

# identify inner users with an indicator
df.socialnetwork$inner <- +(df.socialnetwork$user_id_following %in% inner_id)
df.socialnetwork <- df.socialnetwork[df.socialnetwork$inner == 1]
...
saveRDS(df.friendnum, file = "Data/Temp/socialnetwork_state_inner.rds")
...
saveRDS(df.friendweights, file = 'Data/Temp/friendweight_inner')
...
df.vaxdist <- fread("Data/Region/distribution_by_state.csv",
                            select=c("Date", "Location", "Dist_Per_100K"))
...
colnames(df.vaxdist.wide) <- sub("Dist_Per_100K.","",colnames(df.vaxdist.wide))
...
# save data frames
saveRDS(df.cumulative.dist.wide, file = "Data/Temp/df_cumulative_distribution_state_inner.rds")
saveRDS(df.1diff.dist.wide, file = "Data/Temp/df.1diff.distribution_state_inner.rds")
...
saveRDS(df.friendvax.dist, file = 'Data/Temp/friendvax_state_inner.rds')
...
# save wide table
saveRDS(df.state.dist, file = 'Data/Final Data/wide table_N=92356_v0629_inner.rds')
...
saveRDS(df.state.dist.long, file = 'Data/Final Data/long table_N=92356_v0629_inner.rds')
