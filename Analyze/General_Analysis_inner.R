...
# Liang Yongyin - 2023.4.2 (summarise); Chen Keyu, Shuai Yinzhong, Cui Xiaotong
# Cui Xiaotong - 2023.6.22 (fix bugs)

# you can directly jump to any analysis after running the set up
...
# import data
df.wide <- readRDS('Data/Final Data/wide table_N=92356_v0629_inner.rds')
df.long <- readRDS('Data/Final Data/long table_N=92356_v0629_inner.rds')

# create first lag
df.long <- transform(df.long, friendvax_lag1 = ave(friendvax, user_id, FUN = function(x) c(NA, head(x, -1))))
...
# Generate tables
st(df.wide, file="Results/Description/summary_statistics_inner")
st(df.st, out='latexpage', file="Results/Description/summary_statistics_inner.tex")
...
# ggsave('Results/summary statistics_inner.png', width = 8.5, height = 5, dpi = 300)
...
df.baby <- transform(df.baby, friendvax_lag1 = ave(friendvax, user_id, FUN = function(x) c(NA, head(x, -1))))
df.baby <- transform(df.baby, friendvax_lag2 = ave(friendvax_lag1, user_id, FUN = function(x) c(NA, head(x, -1))))
df.baby <- transform(df.baby, friendvax_lag3 = ave(friendvax_lag2, user_id, FUN = function(x) c(NA, head(x, -1))))
...
df.main <- transform(df.main, friendvax_lag2 = ave(friendvax_lag1, user_id, FUN = function(x) c(NA, head(x, -1))))
df.main <- transform(df.main, friendvax_lag3 = ave(friendvax_lag2, user_id, FUN = function(x) c(NA, head(x, -1))))
df.main <- transform(df.main, friendvax_lag4 = ave(friendvax_lag3, user_id, FUN = function(x) c(NA, head(x, -1))))
df.main <- transform(df.main, friendvax_lag5 = ave(friendvax_lag4, user_id, FUN = function(x) c(NA, head(x, -1))))
df.main <- transform(df.main, friendvax_lag6 = ave(friendvax_lag5, user_id, FUN = function(x) c(NA, head(x, -1))))
df.main <- transform(df.main, friendvax_lag7 = ave(friendvax_lag6, user_id, FUN = function(x) c(NA, head(x, -1))))
...
# output table
table.main.reg <- 
  stargazer(m1, m2, m3, m5, m6, m7, m8, m9, m10, m11,
          ...
          add.lines=list(c("State FE           & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes"),
                         c("Week FE            & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes"),
                         c("Individual Control &     & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes"),
                         c("Network Control    &     &     & Yes & Yes & Yes & Yes & Yes & Yes & Yes & Yes")),
          title = "The effect of social network on vaccination",
          type = "latex",
          out = "Results/Regression/main_20230629_inner")
...
for (state in kStatesAbb) {
  cat(state)
  df.state <- df.heter[df.heter$admin1Abb == state, ]
  h <- glm(vax ~ friendvax_lag1 +
                 friendnum + friendnum_US + 
                 factor(gender) + factor(age) + factor(race) + 
                 factor(week), 
               family = binomial("logit"), data = df.state)
...
ggplot(df.coefplot.state, 
  ...
  geom_text(aes(x="DC", label="ALL", y=100), colour="darkred", angle=90) + 
...
# plot
ggplot(subset(df.coefplot.week, week >= 8 & week < 33), 
       aes(x = week, y = coef)) + 
  geom_point() + 
  geom_hline(yintercept = 37.97, color = "darkgrey", linetype = "longdash") +
  geom_text(aes(x=32, label="ALL", y=50), colour="darkred", angle=0) + 
  geom_hline(yintercept = 0, color = "darkgrey") +
  geom_pointrange(aes(ymin = lower, ymax = upper)) +
...
df.early <- df.heter[df.heter$week %in% c(8, 9, 10),]
...
h.early2 <- glm(vax ~ friendvax_lag1 + 
                friendnum + friendnum_US + 
                factor(gender) + factor(age) + factor(race) + 
                factor(admin1Abb), 
                family = binomial("logit"), data = df.early)
...
table.main.reg <- 
  stargazer(h.early1, h.early2, h.early3, h.early4, h.early,
            ...
            add.lines=list(c("State FE           &     & Yes & Yes & Yes & Yes"),
                           ...
                           c("Individual Control & Yes & Yes &     & Yes & Yes"),
                           c("Network Control    & Yes & Yes & Yes &     & Yes")),
            title = "The effect of social network on vaccination (Jan. 2021)",
            type = "text",
            out = "Results/Regression/supplement_earlyweeks_20230629")
...
h2 <- glm(vax ~ friendvax_lag1 * gender +
             friendnum + friendnum_US  + 
             factor(age) + factor(race) + 
             factor(week) + factor(admin1Abb), 
           family = binomial("logit"), data = df.heter)
...
# race
df.heter$minority <- ifelse(df.heter$race == 0, 0, ifelse(!is.na(df.heter$race), 1, NA) )

h40 <- glm(vax ~ friendvax_lag1 +
            friendnum + friendnum_US + 
            factor(gender) + factor(age) + 
            factor(week) + factor(admin1Abb), 
           family = binomial("logit"), data = df.heter[df.heter$minority == 0,])
...
df.far <- readRDS('Data/Final Data/far-friend-state-sample_long.rds')
df.close <- readRDS('Data/Final Data/close-friend-state-sample_long.rds')

# setDT not working
# setDT(df.far)[, friendvax_lag1 := lag(friendvax), user_id]
# setDT(df.close)[, friendvax_lag1 := lag(friendvax), user_id]

df.far <- transform(df.far, friendvax_lag1 = ave(friendvax, user_id, FUN = function(x) c(NA, head(x, -1))))
df.close <- transform(df.close, friendvax_lag1 = ave(friendvax, user_id, FUN = function(x) c(NA, head(x, -1))))

f20 <- glm(vax ~ friendvax_lag1 +
             friendnum + friendnum_US + 
             factor(gender) + factor(age) + factor(race) + 
             factor(week) + factor(admin1Abb), 
           family = binomial("logit"), data = df.close)

f21 <- glm(vax ~ friendvax_lag1 +
             friendnum + friendnum_US + 
             factor(gender) + factor(age) + factor(race) + 
             factor(week) + factor(admin1Abb), 
           family = binomial("logit"), data = df.far)
...
df.mutual <- transform(df.mutual, friendvax_lag1 = ave(friendvax, user_id, FUN = function(x) c(NA, head(x, -1))))
...
df.nocel <- transform(df.nocel, friendvax_lag1 = ave(friendvax, user_id, FUN = function(x) c(NA, head(x, -1))))
df.cele <- transform(df.cele, friendvax_lag1 = ave(friendvax, user_id, FUN = function(x) c(NA, head(x, -1))))
...
# heterogeneity in social network
...
f20.b <- summary(f20)$coefficients["friendvax_lag1",][[1]]
f20.se <- summary(f20)$coefficients["friendvax_lag1",][[2]]
df.coefplot <- rbind(df.coefplot, c("Network2", "Close", f20.b, f20.b - 1.96*f20.se, f20.b + 1.96*f20.se))

f21.b <- summary(f21)$coefficients["friendvax_lag1",][[1]]
f21.se <- summary(f21)$coefficients["friendvax_lag1",][[2]]
df.coefplot <- rbind(df.coefplot, c("Network2", "Distant", f21.b, f21.b - 1.96*f21.se, f21.b + 1.96*f21.se))

f3.b <- summary(f3)$coefficients["friendvax_lag1",][[1]]
f3.se <- summary(f3)$coefficients["friendvax_lag1",][[2]]
df.coefplot <- rbind(df.coefplot, c("Network3", "Mutual Friends", f3.b, f3.b - 1.96*f3.se, f3.b + 1.96*f3.se))

f40.b <- summary(f40)$coefficients["friendvax_lag1",][[1]]
f40.se <- summary(f40)$coefficients["friendvax_lag1",][[2]]
df.coefplot <- rbind(df.coefplot, c("Network4", "Non-Celebrity", f40.b, f40.b - 1.96*f40.se, f40.b + 1.96*f40.se))

f41.b <- summary(f41)$coefficients["friendvax_lag1",][[1]]
f41.se <- summary(f41)$coefficients["friendvax_lag1",][[2]]
df.coefplot <- rbind(df.coefplot, c("Network4", "Celebrity", f41.b, f41.b - 1.96*f41.se, f41.b + 1.96*f41.se))
...
saveRDS(df.coefplot, file = "Results/Regression/coefplot.rds")

# df.coefplot <- readRDS("Results/Regression/coefplot.rds")

# coefficient plot
ggplot(df.coefplot[1:10,], 
       aes(factor(model, level = c("All", "Female", "Male", "Under 18", "19-39", 
                            "Above 40", "White", "Minority", "Blue State", "Red State")), 
           coef, color = type, label = coef)) + 
  geom_point() + 
  geom_text(aes(label = round(coef, digit = 2)), nudge_x = 0.25) +
  geom_hline(yintercept = 37.9741621, color = "darkgrey", linetype = "longdash") + 
  geom_pointrange(aes(ymin = lower, ymax = upper)) +
  scale_x_discrete(limits=rev) + 
...
ggplot(df.coefplot[c(1, 11:17),], 
       aes(factor(model, level = c("All", "Low USfriend", "High USfriend", "Close", "Distant",   
                                   "Mutual Friends", "Non-Celebrity", "Celebrity")), 
           coef, color = type, label = coef)) + 
  geom_point() + 
  geom_text(aes(label = round(coef, digit = 2)), nudge_x = 0.25) +
  geom_hline(yintercept = 37.9741621, color = "darkgrey", linetype = "longdash") + 
...
table.heter.reg <- 
  stargazer(m3, h20, h21, h30, h31, h32, h40, h41,
            ...
            type = "text",
            out = "Results/Regression/heter_subgroup_v0629_inner")
