#Regression_baseline-state-cond+1diff.R
#Shuai Yinzhong - 2022.12.3
...
# conditional
setDT(df_reg)[, vax_lag1 := lag(vax), user_id]
df_reg <- df_reg[vax_lag1 != 1, ]
...
# create differences
df_reg$friendvax_diff1 <- df_reg$friendvax - df_reg$friendvax_lag1
df_reg$friendvax_diff2 <- df_reg$friendvax_lag1 - df_reg$friendvax_lag2
df_reg$friendvax_diff3 <- df_reg$friendvax_lag2 - df_reg$friendvax_lag3
df_reg$friendvax_diff4 <- df_reg$friendvax_lag3 - df_reg$friendvax_lag4


# baseline logistic model
b.simple <- glm(formula=vax ~ friendvax_diff2 + friendvax_lag2,
                family=binomial("logit"), data=df_reg)

b.FE <- glm(formula=vax ~ friendvax_diff2 + friendvax_lag2 + 
              factor(admin1Abb) + factor(week),
            family=binomial("logit"), data=df_reg)

b.benchmark <- glm(formula=vax ~ friendvax_diff2 + friendvax_lag2 + 
...
# b.2lag = glm(formula=vax ~ friendvax_diff2 + friendvax_diff3 + friendvax_lag3 + 
#                factor(gender) + factor(age) + factor(race) + 
#                factor(admin1Abb) + factor(week),
#              family=binomial("logit"), data=df_reg)

## summary and output
stargazer(b.simple, b.FE, b.benchmark, omit = c("week", "admin1Abb",
                                                        "gender", "age", "wage", "race", "ALLfriend"), 
          covariate.labels = c("FriendVax Diff2", "FriendVax Lag2"),
          ...
          type = "latex",
          out="Results/Regression/benchmark-1diff-cond_20221203")
